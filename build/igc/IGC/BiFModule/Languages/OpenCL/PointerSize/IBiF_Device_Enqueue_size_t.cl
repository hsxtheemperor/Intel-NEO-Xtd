    ...
    clk_event_t event)
{
    ...
    if( offset >= 0 )
    {
        ...
        pCommand->m_event                 = (int)(size_t)__builtin_astype(event, __private void*);
        ...
        if( 0 != (size_t)__builtin_astype(event, __private void*) )
        {
            __global IGIL_DeviceEvent *events = IGIL_GetDeviceEvents();

            atomic_inc( &events[(int)(size_t)__builtin_astype(event, __private void*)].m_numChildren );
        }
    ...
    clk_event_t parentEvent)
{
    ...
    if( 0 != out_event )
    {
        ...
        if( 0 == (size_t)__builtin_astype(myEvent, __private void*) )
        {
          status = CLK_EVENT_ALLOCATION_FAILURE;
        }
        else if( 0 != (size_t)__builtin_astype(parentEvent, __private void*) )
        {
            // if parent event, update its count of children to include newly created event

            events[(int)(size_t)__builtin_astype(myEvent, __private void*)].m_parentEvent = (size_t)__builtin_astype(parentEvent, __private void*);
            atomic_inc( &events[(int)(size_t)__builtin_astype(parentEvent, __private void*)].m_numChildren );
            events[(int)(size_t)__builtin_astype(myEvent, __private void*)].m_state = CL_QUEUED;
        }
    }
    ...
    else
    {
        ...
        if( pCommand != 0 )
        {
            ...
            for( int i = 0; i < numEvents; i++ )
            {
                ...
                atomic_inc( &events[(int)(size_t)__builtin_astype(d, __private void*)].m_numDependents );
            }
        }
        else
        {
            ...
            if( NULL != out_event )
            {
                ...
                myEvent = __builtin_astype((__private void *)0, clk_event_t);
            }
        }
    }
