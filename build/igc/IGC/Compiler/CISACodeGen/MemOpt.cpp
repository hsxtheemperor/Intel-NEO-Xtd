    ...
    TrivialMemRefListTy& ToOpt)
{
    ...
    Type* NewPointerType =
        IGCLLVM::getPointerType(NewLoadType, LeadingLoad->getPointerAddressSpace());
...
bool MemOpt::mergeStore(StoreInst* LeadingStore,
    ...
    // calculations.
    Type* NewPointerType =
        IGCLLVM::getPointerType(NewStoreType, LeadingStore->getPointerAddressSpace());
    Value* NewPointer =
...
void LdStCombine::createCombinedStores(BasicBlock* BB)
{
    for (auto& bundle : m_bundles)
    {
        ...
        PointerType* nPTy = IGCLLVM::getPointerType(VTy, PTy->getAddressSpace());
    ...
    for (auto& bundle : m_bundles)
    {
        ...
        if (anchorLoad != leadLoad && isa<Instruction>(Addr)) {
            ...
            if (MI != m_instOrder.end() && MI->second > anchorLoadNum)
            {
                ...
                Type* nTy = IGCLLVM::getPointerType(bTy, leadLoad->getPointerAddressSpace());
...
bool isLayoutStructType(const StructType* StTy)
{
    ...
    return (stId.starts_with(getStructNameForSOALayout()) ||
        stId.starts_with(getStructNameForAOSLayout()));
}
