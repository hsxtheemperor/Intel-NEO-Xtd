...
namespace
{
    ...
    public:
        ...
        Instruction* Create(Type* returnType, Value* ptr, alignment_t alignment, bool isVolatile)
        {
            ...
            if (isa<LoadInst>(m_inst))
            {
                Type* newPtrType = IGCLLVM::getPointerType(returnType, ptr->getType()->getPointerAddressSpace());
        ...
        Value* CreateConstScalarGEP(Type* returnType, Value* ptr, uint32_t offset)
        {
            ...
            if (isa<LoadInst>(m_inst))
            {
                Type* ePtrType = IGCLLVM::getPointerType(returnType->getScalarType(), ptr->getType()->getPointerAddressSpace());
    ...
    public:
        ...
        Instruction* Create(Value* storedValue, Value* ptr, alignment_t alignment, bool isVolatile)
        {
            ...
            if (isa<StoreInst>(m_inst))
            {
                Type* newPtrType = IGCLLVM::getPointerType(newType, ptr->getType()->getPointerAddressSpace());
                ptr = builder.CreateBitCast(ptr, newPtrType);
                return alignment ?
        ...
        Value* CreateConstScalarGEP(Type* storedType, Value* ptr, uint32_t offset)
        {
            ...
            if (isa<StoreInst>(m_inst))
            {
                Type* ePtrType = IGCLLVM::getPointerType(storedType->getScalarType(), ptr->getType()->getPointerAddressSpace());
                ptr = builder.CreateBitCast(ptr, ePtrType);
    ...
    Instruction* Inst, V2SMap& vecToSubVec, WIAnalysisRunner& WI)
{
    ...
    if (aALI)
    {
        splitLoad(aALI.value(), vecToSubVec, WI);
    }

    if (ASI)
    {
        splitStore(ASI.value(), vecToSubVec, WI);
    }
...
Instruction* VectorPreProcess::simplifyLoadStore(Instruction* Inst)
{
    if (Optional<AbstractLoadInst> optionalALI = AbstractLoadInst::get(Inst, *m_DL))
    {
        ...
        AbstractLoadInst& ALI = optionalALI.value();
    ...
    Optional<AbstractStoreInst> optionalASI = AbstractStoreInst::get(Inst, *m_DL);
    AbstractStoreInst& ASI = optionalASI.value();
...
bool VectorPreProcess::processScalarLoadStore(Function& F)
{
    ...
    for (inst_iterator I = inst_begin(F), E = inst_end(F); I != E; ++I)
    {
        ...
        if (isa<StoreInst>(inst))
        {
            ...
            Type* newPtrType = IGCLLVM::getPointerType(newVecTy, ptr->getType()->getPointerAddressSpace());
