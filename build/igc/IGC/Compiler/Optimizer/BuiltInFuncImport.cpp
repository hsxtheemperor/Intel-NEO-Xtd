...
bool BIImport::runOnModule(Module& M)
{
    ...
    std::function<void(Function*)> Explore = [&](Function* pRoot)
    {
        ...
        for (auto* pCallee : calledFuncs)
        {
            ...
            if (pFunc->getName().starts_with("__builtin_IB_kmp_"))
            {
        ...
        else if (funcName.starts_with("__builtin_IB_call_function_pointer"))
        {
            for (auto user : F.users())
            {
                if (CallInst* CI = dyn_cast<CallInst>(&*user))
                {
                    ...
                    Value* funcPtr = builder.CreatePointerCast(CI->getArgOperand(0), IGCLLVM::getPointerType(funcTy, 0));
...
bool PreBIImportAnalysis::runOnModule(Module& M)
{
    ...
    for (Module::iterator I = M.begin(), E = M.end(); I != E; ++I)
    {
                ...
                for (; instUse != instEnd; ++instUse) {
                    if (CallInst* useInst =
                        ...
                        if (!funcName.starts_with(cosBuiltinName) &&
                            !funcName.starts_with(sinBuiltinName) &&
                            !funcName.starts_with(sinPiBuiltinName) &&
                            !funcName.starts_with(cosPiBuiltinName)) {
                            return true;
                        }
                    }
                    else
        ...
        if ((modMD->compOpt.MatchSinCosPi) &&
            ...
            (funcName.starts_with(cosBuiltinName) ||
             funcName.starts_with(sinBuiltinName))) {
