...
void CImagesBI::prepareImageBTI()
{
    ...
    Value* img = ConstantPointerNull::get(IGCLLVM::getPointerType(pImg->getType(), addrSpace));
...
void CImagesBI::preparePairedResource()
{
    ...
    IGC_ASSERT(isa<Argument>(pImg) || isa<LoadInst>(pImg));
    unsigned int addrSpace = EncodeAS4GFXResource(*imageIndex, RESOURCE);
    Type* ptrTy = IGCLLVM::getPointerType(m_pFloatType, addrSpace);
...
void CImagesBI::createGetBufferPtr()
{
    ...
    Type* ptrTy = IGCLLVM::getPointerType(m_pFloatType, addressSpace);
...
public:
    ...
    void CreateInlineSamplerAnnotations(InlineSamplersMD& inlineSamplerMD, int samplerValue)
    {
        ...
        if (llvm::StringRef(M->getTargetTriple()).starts_with("igil") || llvm::StringRef(M->getTargetTriple()).starts_with("gpu_64"))
        {
    ...
    bool prepareSamplerValue()
    {
        ...
        if (isa<ConstantInt>(samplerValue))
        {
            ...
            samplerValue = ConstantPointerNull::get(IGCLLVM::getPointerType(samplerValue->getType(), addrSpace));
        }
