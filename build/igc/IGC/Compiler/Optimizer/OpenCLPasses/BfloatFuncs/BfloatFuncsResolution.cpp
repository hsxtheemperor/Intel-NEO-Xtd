...
void BfloatFuncsResolution::visitCallInst(CallInst &CI) {
    ...
    std::string DNameStr =
        llvm::demangle(CI.getCalledFunction()->getName().str());
    ...
    llvm::StringSwitch<std::function<void()>>(DName)
        ...
        .StartsWith("__builtin_bf16_sub",
                    [&]() { handleArithmetic(CI, Instruction::FSub); })
        .StartsWith("__builtin_bf16_mul",
                    [&]() { handleArithmetic(CI, Instruction::FMul); })
        .StartsWith("__builtin_bf16_mad",
                    [&]() { handleArithmetic(CI, -1, true); })

        .StartsWith("__builtin_bf16_log",
                    [&]() { handleMath(CI, Intrinsic::log2); })
        .StartsWith("__builtin_bf16_exp",
                    [&]() { handleMath(CI, Intrinsic::exp2); })
        .StartsWith("__builtin_bf16_sqrt",
                    [&]() { handleMath(CI, Intrinsic::sqrt); })
        .StartsWith("__builtin_bf16_sin",
                    [&]() { handleMath(CI, Intrinsic::sin); })
        .StartsWith("__builtin_bf16_cos",
                    [&]() { handleMath(CI, Intrinsic::cos); })
        .StartsWith("__builtin_bf16_inv",
                    [&]() { handleMath(CI, GenISAIntrinsic::GenISA_inv, true); })
        .Default([&]() {
          IGC_ASSERT(0);
...
// return <2 x bfloat>
Type *BfloatFuncsResolution::getTypeBasedOnType(Type *OrgType,
                                                Type *DesiredTypeScalar) {
    auto DestType =
        OrgType->isVectorTy()
...
Value* BfloatFuncsResolution::bitcastToBfloat(Value *V) {
    IGC_ASSERT(V && V->getType()->getScalarSizeInBits() ==
                        m_builder->getBFloatTy()->getScalarSizeInBits());
...
void BfloatFuncsResolution::handleCompare(CallInst &CI, CmpInst::Predicate Pred) {
    ...
    if (IGCLLVM::getNumArgOperands(&CI) != 2 ||
        !Op0->getType()->getScalarType()->isIntegerTy(16) ||
        !Op1->getType()->getScalarType()->isIntegerTy(16) ||
        !CI.getType()->getScalarType()->isIntegerTy(32)) {
...
void BfloatFuncsResolution::handleSelect(CallInst &CI) {
    ...
    if (IGCLLVM::getNumArgOperands(&CI) != 3 ||
        !CondOp->getType()->getScalarType()->isIntegerTy() ||
        !Op1->getType()->getScalarType()->isIntegerTy(16) ||
        !Op2->getType()->getScalarType()->isIntegerTy(16) ||
    ...
    Type *SelectType =
        CondOp->getType()->isVectorTy()
            ? IGCLLVM::FixedVectorType::get(
                  m_builder->getInt1Ty(),
                  (unsigned)cast<IGCLLVM::FixedVectorType>(CondOp->getType())
                      ->getNumElements())
...
void BfloatFuncsResolution::handleMinMax(CallInst &CI,
...
void BfloatFuncsResolution::handleArithmetic(llvm::CallInst& CI,
  unsigned Opcode, bool IsMadInstruction) {
    ...
    for (unsigned i = 0; i < IGCLLVM::getNumArgOperands(&CI); ++i) {
      ...
      if (Op->getType()->getScalarType()->isFloatTy()) {
        ...
        m_ctx->EmitError("Arguments expected to be either float or short!",
                         &CI);
        return;
      }
...
void BfloatFuncsResolution::handleMath(
    llvm::CallInst &CI, llvm::Intrinsic::ID Operation, bool needsGenISAIntrinsic) {
    auto Op0 = CI.getOperand(0);

    if (IGCLLVM::getNumArgOperands(&CI) != 1 ||
        !Op0->getType()->getScalarType()->isIntegerTy(16) ||
        !CI.getType()->getScalarType()->isIntegerTy(16)) {
      m_ctx->EmitError("Unexpected function signature", &CI);
      return;
