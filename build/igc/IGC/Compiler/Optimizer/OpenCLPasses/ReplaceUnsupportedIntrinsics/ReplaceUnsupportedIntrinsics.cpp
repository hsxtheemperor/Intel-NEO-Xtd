...
void ReplaceUnsupportedIntrinsics::replaceMemcpy(IntrinsicInst* I)
{
    ...
    if (CI)
    {
        ...
        if (NewCount > 0)
        {
            vSrc = Builder.CreateBitCast(SkipBitCast(Src), IGCLLVM::getPointerType(VecTys[0], SrcAS), "memcpy_vsrc");
            vDst = Builder.CreateBitCast(SkipBitCast(Dst), IGCLLVM::getPointerType(VecTys[0], DstAS), "memcpy_vdst");
        ...
        for (unsigned i = 1; i < Len; ++i)
        {
            ...
            vSrc = Builder.CreateBitCast(SkipBitCast(NewSrc), IGCLLVM::getPointerType(VecTys[i], SrcAS), "memcpy_rem");
            vDst = Builder.CreateBitCast(SkipBitCast(NewDst), IGCLLVM::getPointerType(VecTys[i], DstAS), "memcpy_rem");
    ...
    if (CI)
    {
        ...
        // for true block (Src < Dst), do a reverse copy.
        {
            ...
            for (unsigned i = Len - 1; i >= 1; i--)
            {
                ...
                auto* vSrc = B.CreateBitCast(SkipBitCast(tSrc), IGCLLVM::getPointerType(VecTys[i], SrcAS), "memcpy_rem");
                auto* vDst = B.CreateBitCast(SkipBitCast(tDst), IGCLLVM::getPointerType(VecTys[i], DstAS), "memcpy_rem");
            ...
            auto* vSrc = B.CreateBitCast(SkipBitCast(Src), IGCLLVM::getPointerType(VecTys[0], SrcAS), "memcpy_vsrc");
            auto* vDst = B.CreateBitCast(SkipBitCast(Dst), IGCLLVM::getPointerType(VecTys[0], DstAS), "memcpy_vdst");
            // If NewCount is less than the threshold, don't generate loop.
            uint32_t SZ = (unsigned int)(VecTys[0]->getPrimitiveSizeInBits() / 8);
...
void ReplaceUnsupportedIntrinsics::replaceMemset(IntrinsicInst* I)
{
    ...
    if (CI)
    {
        ...
        if (NewCount > 0)
        {
            PointerType* PTy = IGCLLVM::getPointerType(VecTys[0], AS);
        ...
        for (unsigned i = 1; i < Len; ++i)
        {
            ...
            uint32_t adjust_align = getLargestPowerOfTwo(SZ);
            Align = adjust_align < Align ? adjust_align : Align;
            PointerType* PTy = IGCLLVM::getPointerType(VecTys[i], AS);
            NewDst = BOfst > 0 ? Builder.CreateConstGEP1_32(Builder.getInt8Ty(), Dst, BOfst) : Dst;
