    ...
    for (auto& F : module)
    {
        if (F.isDeclaration() && !F.isIntrinsic() && !GenISAIntrinsic::isIntrinsic(&F) && F.hasNUsesOrMore(1))
        {
            ...
            if (!funcName.starts_with("__builtin_IB") && funcName != "printf" &&
                !funcName.starts_with("__builtin_bf16") &&
                !funcName.starts_with("__igcbuiltin_") &&
                !funcName.starts_with("__translate_sampler_initializer") &&
#if defined(IGC_SCALAR_USE_KHRONOS_SPIRV_TRANSLATOR)
                !funcName.starts_with("_Z20__spirv_SampledImage") &&
                !funcName.starts_with("_Z21__spirv_VmeImageINTEL") &&
#else
                !funcName.starts_with("__builtin_spirv_OpSampledImage") &&
                !funcName.starts_with("__builtin_spirv_OpVmeImageINTEL") &&
#endif
