...
#include "WrapperLLVM/include/llvmWrapper/ADT/StringExtras.h"
...
using DILexicalBlockFile = llvm::DILexicalBlockFile;
...
void DbgVariable::emitExpression(CompileUnit *CU, DIEBlock *Block) const {
  IGC_ASSERT(CU);
  IGC_ASSERT(Block);
...
void DwarfDebug::endSections() {
  ...
  for (SectionMapType::iterator it = SectionMap.begin(); it != SectionMap.end(); ++it) {
    const ::llvm::MCSection *Section = it->first;
  ...
  if (!LScopes.empty()) {
    ...
    for (unsigned i = 0, e = AList.size(); i != e; ++i) {
      LexicalScope *AScope = AList[i];
        const llvm::DISubprogram *SubProg = 
          llvm::cast_or_null<llvm::DISubprogram>(AScope->getScopeNode());
      if (SubProg) {
        // Collect info for variables that were optimized out.
        auto Variables = SubProg->getRetainedNodes();
        for (unsigned i = 0, e = Variables.size(); i != e; ++i) {
          DILocalVariable *DV = cast_or_null<DILocalVariable>(Variables[i]);
...
void DwarfDebug::recordSourceLine(unsigned Line, unsigned Col, const MDNode *S,
  ...
  unsigned Src = 1;
if (S) {
    if (isa<llvm::DICompileUnit>(S)) {
      const llvm::DICompileUnit *CU = cast<llvm::DICompileUnit>(S);
      Fn = CU->getFilename();
      Dir = CU->getDirectory();
    } else if (isa<llvm::DIFile>(S)) {
      const llvm::DIFile *F = cast<llvm::DIFile>(S);
      Fn = F->getFilename();
      Dir = F->getDirectory();
    } else if (isa<llvm::DISubprogram>(S)) {
      // Renamed SP to SubProg to avoid collision with Stack Pointer register
      const llvm::DISubprogram *SubProg = llvm::cast<llvm::DISubprogram>(S);
      Fn = SubProg->getFilename();
      Dir = SubProg->getDirectory();
    } else if (isa<llvm::DILexicalBlockFile>(S)) {
      const llvm::DILexicalBlockFile *DBF = cast<llvm::DILexicalBlockFile>(S);
      Fn = DBF->getFilename();
      Dir = DBF->getDirectory();
    } else if (isa<llvm::DILexicalBlock>(S)) {
      const llvm::DILexicalBlock *DB = cast<llvm::DILexicalBlock>(S);
      Fn = DB->getFilename();
      Dir = DB->getDirectory();
    } else {
