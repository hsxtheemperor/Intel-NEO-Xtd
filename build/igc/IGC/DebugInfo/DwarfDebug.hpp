...
#include "WrapperLLVM/include/llvmWrapper/IR/IntrinsicInst.h"
...
public:
  ...
  void emitExpression(CompileUnit *CU, DIEBlock *Block) const;
...
class DwarfDebug {
  ...
  ::std::vector<::std::unique_ptr<::IGC::DbgVariable>> DbgVariablesStorage;
...
public:
  ...
  // Main entry points.
  //
  DwarfDebug(StreamEmitter *A, ::IGC::VISAModule *M);

  StreamEmitter &getStreamEmitter() const { return *Asm; }

  const DebugEmitterOpts &getEmitterSettings() const {
    return EmitSettings;
  }
  ...
  void beginFunction(const ::llvm::Function *MF, VISAModule *);
  ...
  void registerVISA(VISAModule *M);
...
private:
  ...
  const VISAObjectDebugInfo *VisaDbgInfo = nullptr;
...
public:
  ...
  const VISAObjectDebugInfo &getVisaDebugInfo() {
    IGC_ASSERT(VisaDbgInfo);
    return *VisaDbgInfo;
  }

  void setVisaDbgInfo(const VISAObjectDebugInfo &VDI) {
    VisaDbgInfo = &VDI;
  }
