...
namespace {
  ...
  private:
    ...
    void recordCopyCandidate(SimpleValue Dest, unsigned OperandIndex,
      ...
      IGC_ASSERT(isa<User>(Dest.value()));
      recordCandidate(Dest, &cast<User>(Dest.value())->getOperandUse(OperandIndex),
...
{
  IGC_ASSERT(isa<Instruction>(SV.value()));
  auto *BB = cast<Instruction>(SV.value())->getParent();
  auto *Type = IndexFlattener::getElementType(SV.value()->getType(), SV.getIndex());
...
{
  LLVM_DEBUG(dbgs() << "Trying to record cand " << *(Dest.value()) << "\n");
  if (UseInDest && (isa<UndefValue>(*UseInDest) || isa<Constant>(*UseInDest)))
    return;
  LLVM_DEBUG(dbgs() << "Recording cand " << *(Dest.value()) << "\n");
...
void GenXCoalescing::processCandidate(const Candidate &Cand, bool IsCopy)
{
  ...
  SimpleValue Source;
  if (!Cand.UseInDest) {
    auto *Callee = cast<CallInst>(Dest.value())->getCalledFunction();
  ...
  LLVM_DEBUG(dbgs() << "Trying coalesce from ";
      ...
      if (isa<Argument>(Dest.value()))
        dbgs() << " (call arg)";
      else if (Liveness->isUnifiedRet(Dest.value()))
        dbgs() << " (ret pre-copy)";
  ...
  // are not constant.
  IGC_ASSERT(!Cand.UseInDest || !isa<Constant>(Source.value()) || isa<UndefValue>(Source.value()));
  ...
  if (SourceLR && SourceLR->Category == DestLR->Category) {
    if (IsCopy) {
      ...
      if (!Liveness->copyInterfere(SourceLR, DestLR)) {
        ...
        if (auto *CI = dyn_cast<CastInst>(Dest.value());
            CI && genx::isNoopCast(CI)) {
          CopyCoalesced[CI] = Source.value();
        }
        return;
      }
    } else {
      ...
      if (!Liveness->twoAddrInterfere(DestLR, SourceLR)) {
        ...
        Liveness->coalesce(DestLR, SourceLR,
            /*DisallowCASC=*/ !isa<Argument>(Dest.value()));
        return;
      }
    }
  }
    ...
    if (!DestLR->DisallowCASC) {
      ...
      for (auto ui = Callee->use_begin(), ue = Callee->use_end();
        ...
        auto OtherArg = SimpleValue(CallSite->getArgOperand(cast<Argument>(
                Dest.value())->getArgNo()), Dest.getIndex());
  ...
  if (auto *CI = dyn_cast<CastInst>(Dest.value());
    ...
    unsigned TySz = vc::getTypeSize(Dest.value()->getType(), DL).inBytes();
  ...
  Instruction *DestInst = cast<Instruction>(Dest.value());
...
{
  IGC_ASSERT(!isa<Constant>(Input.value()));
  if (auto ST = dyn_cast<StructType>(Input.value()->getType())) {
    ...
    Instruction *Extract = ExtractValueInst::Create(Input.value(), Indices,
  ...
  ++NumInsertedCopies;
  return Liveness->insertCopy(Input.value(), LR, InsertBefore, Name, Number,
                              ST);
}
...
LiveRange *GenXCoalescing::mergeCopiesTillFailed(SimpleValue CopySV, Iter &It,
  ...
  Instruction *CurrCopy = cast<Instruction>(CopySV.value());
  ...
  while (It != EndIt) {
    ...
    if (!getDomTree(CurrCopy->getFunction())
             ->dominates(CurrCopy, cast<Instruction>(It->Dest.value()))) {
    ...
    if (It->Source.value()->getType() == CurrCopy->getType()) {
      *It->UseInDest = CurrCopy;
      ...
      BCI = cast<BitCastInst>(Builder.CreateBitCast(
          CurrCopy, It->Source.value()->getType(), "red_copy_type_conv"));
...
SortedCopies GenXCoalescing::getSortedCopyData() {
  ...
  std::for_each(ToCopy.begin(), ToCopy.end(), [&](auto &CD) {
    ...
    Value *SourceVal = CD.Source.value();
...
Instruction *GenXCoalescing::createCopy(const CopyData &CD) {
  ...
  case PHICOPY_BRANCHING_JP: {
    PHINode *Phi = dyn_cast<PHINode>(CD.Dest.value());
  ...
  case TWOADDRCOPY: {
    ...
    // instruction.
    Instruction *DestInst = cast<Instruction>(CD.Dest.value());
...
void GenXCoalescing::replaceAllUsesWith(Instruction *OldInst,
  ...
  ToCopy.erase(std::remove_if(ToCopy.begin(), ToCopy.end(),
                              [&](CopyData const &CD) {
                                if (CD.Dest.value() == OldInst)
                                  return true;
                                if (CD.Source.value() == OldInst)
                                  return true;
                                return false;
                              }),
               ToCopy.end());
