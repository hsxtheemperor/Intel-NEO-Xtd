...
void GenXSimdCFConformance::gatherRMVals() {
  ...
  for (auto ji = EMVals.begin(), je = EMVals.end(); ji != je; ++ji) {
    auto EMVal = *ji;
    auto IID = vc::getAnyIntrinsicID(EMVal.value());
    if (IID != GenXIntrinsic::genx_simdcf_join)
      continue;
    auto Join = cast<CallInst>(EMVal.value());
...
void GenXSimdCFConformance::moveCodeInGotoBlocks(bool hoistGotoUsers) {
  ...
  for (auto gi = EMVals.begin(), ge = EMVals.end(); gi != ge; ++gi) {
    ...
    if (IID != GenXIntrinsic::genx_simdcf_goto)
      continue;
    auto Goto = cast<CallInst>(EMVal.value());
...
void GenXSimdCFConformance::moveCodeInJoinBlocks() {
  ...
  for (auto ji = EMVals.begin(), je = EMVals.end(); ji != je; ++ji) {
    ...
    if (IID != GenXIntrinsic::genx_simdcf_join)
      continue;
    Joins.push_back(cast<CallInst>(EMVal.value()));
  }
...
void GenXSimdCFConformance::ensureConformance() {
  ...
  for (;;) {
    if (!EMValsStack.empty()) {
      ...
      if (!EMVal.getIndex()) {
        if (auto *CI = dyn_cast<CallInst>(EMVal.value())) {
          auto IID = vc::getAnyIntrinsicID(EMVal.value());
          switch (IID) {
          case GenXIntrinsic::genx_simdcf_goto:
...
bool GenXSimdCFConformance::checkEMVal(SimpleValue EMVal) {
  LLVM_DEBUG(dbgs() << "checkEMVal: " << *EMVal.value() << "#"
                    << EMVal.getIndex() << "\n");
  ...
  for (auto i = ConnectedVals.begin(), e = ConnectedVals.end(); i != e; ++i) {
    ...
    if (auto *C = dyn_cast<Constant>(ConnectedVal.value())) {
      if (!C->isAllOnesValue()) {
        LLVM_DEBUG(
            dbgs()
...
bool GenXSimdCFConformance::checkJoin(SimpleValue EMVal) {
  ...
  for (unsigned rvi = 0; rvi != RMValsEntry->size(); ++rvi) {
    ...
    LLVM_DEBUG(
        dbgs() << "checkJoin: getConnectedVals-> " << RM.value()->getName()
               << "#" << RM.getIndex() << "\n";
      ...
      if (auto *C = dyn_cast<Constant>(ConnectedVal.value())) {
        ...
        if (RMValsEntry->insert(ConnectedVal)) {
          LLVM_DEBUG(dbgs() << "checkJoin: New one: "
                            << ConnectedVal.value()->getName() << "#"
                            << ConnectedVal.getIndex() << "\n");
          auto IID = vc::getAnyIntrinsicID(ConnectedVal.value());
          switch (IID) {
          case GenXIntrinsic::genx_simdcf_join:
  ...
  for (unsigned rvi = 0; rvi != RMValsEntry->size(); ++rvi) {
    SimpleValue RM = (*RMValsEntry)[rvi];
    auto IID = vc::getAnyIntrinsicID(RM.value());
    if (IID == GenXIntrinsic::genx_simdcf_goto)
      GotoJoinMap[cast<CallInst>(RM.value())] = Join;
  }
  return true;
}
...
bool GenXSimdCFConformance::checkGotoJoin(SimpleValue EMVal) {
  auto CI = cast<CallInst>(EMVal.value());
...
bool GenXSimdCFConformance::getConnectedVals(
    ...
    bool LowerBadUsers) {
  // Check the def first.
  if (auto *Arg = dyn_cast<Argument>(Val.value())) {
    ...
    auto ValTy = IndexFlattener::getElementType(Val.value()->getType(),
                                                Val.getIndex());
  ...
  } else if (auto *Phi = dyn_cast<PHINode>(Val.value())) {
  ...
  } else if (auto *EVI = dyn_cast<ExtractValueInst>(Val.value())) {
    ...
    ConnectedVals->push_back(SimpleValue(
        ...
        Val.getIndex() + IndexFlattener::flatten(
                             ...
                             EVI->getIndices())));
  } else if (auto *IVI = dyn_cast<InsertValueInst>(Val.value())) {
  ...
  } else if (auto *SVI = dyn_cast<ShuffleVectorInst>(Val.value())) {
    // shufflevector: add the EM use
    ConnectedVals->push_back(SimpleValue(SVI->getOperand(0), 0));
  } else if (auto *CI = dyn_cast<CallInst>(Val.value())) {
    ...
    case GenXIntrinsic::genx_simdcf_goto:
      ...
      if (Cat == vc::RegCategory::EM)
        if (auto Join = GotoJoinMap[cast<CallInst>(Val.value())])
          ConnectedVals->push_back(
    ...
    case GenXIntrinsic::genx_simdcf_join: {
      ...
      auto RMValsEntry = &RMVals[cast<CallInst>(Val.value())];
  ...
  LLVM_DEBUG(dbgs() << "getConnectedVals: get all uses for " << *Val.value()
                    << "\n");
  for (auto ui = Val.value()->use_begin(), ue = Val.value()->use_end();
       ui != ue; ++ui) {
    auto User = cast<Instruction>(ui->getUser());
  ...
  if (LowerBadUsers) {
    ...
    for (auto &BadUser : UsersToLower) {
      replaceUseWithLoweredEM(dyn_cast<Instruction>(BadUser.value()),
...
void GenXSimdCFConformance::resolveBitCastChains() {
  ...
  for (auto &Val : EMVals) {
    if (auto *PN = dyn_cast<PHINode>(Val.value())) {
      LLVM_DEBUG(dbgs() << "resolveBitCastChains: Found phi:\n" << *PN << "\n");
    } else if (auto *BCI = dyn_cast<BitCastInst>(Val.value())) {
    ...
    std::set<Value *> Visited;
    Instruction *I = dyn_cast<Instruction>(Val.value());
...
void GenXLateSimdCFConformance::setCategories() {
  ...
  for (auto ei = EMVals.begin(); ei != EMVals.end(); /* empty */) {
    ...
    if (!isa<StructType>(EMVal.value()->getType()))
      modifyEMUses(EMVal.value());
    switch (vc::getAnyIntrinsicID(EMVal.value())) {
    case GenXIntrinsic::genx_simdcf_join: {
      // For a join, set the category of each RM value.
      auto RMValsEntry = &RMVals[cast<CallInst>(EMVal.value())];
...
void GenXLateSimdCFConformance::hoistExtractEMInstructions() {
  ...
  for (auto &&EMVal : EMVals)
    if (auto *V = dyn_cast<ExtractValueInst>(EMVal.value())) {
