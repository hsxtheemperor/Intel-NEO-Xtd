...
DecodeIITType(unsigned &NextElt, ArrayRef<unsigned char> Infos,
  ...
  case IIT_HALF_VEC_ARG: {
    unsigned ArgInfo = (NextElt == Infos.size() ? 0 : Infos[NextElt++]);
    OutputTable.push_back(
        IITDescriptor::get(IITDescriptor::Vector, ArgInfo));
    return;
  }
  ...
  case IIT_PTR_TO_ARG: {
    unsigned ArgInfo = (NextElt == Infos.size() ? 0 : Infos[NextElt++]);
    OutputTable.push_back(
        IITDescriptor::get(IITDescriptor::Pointer, ArgInfo));
    return;
  }
  case IIT_PTR_TO_ELT: {
    unsigned ArgInfo = (NextElt == Infos.size() ? 0 : Infos[NextElt++]);
    OutputTable.push_back(IITDescriptor::get(IITDescriptor::Pointer, ArgInfo));
    return;
  }
...
static Type *DecodeFixedType(ArrayRef<Intrinsic::IITDescriptor> &Infos,
  ...
  case IITDescriptor::Vector:
    // If we have a width in the descriptor, it's a fixed vector
    if (D.Kind == IITDescriptor::Vector) {
        return VectorType::get(DecodeFixedType(Infos, Tys, Context), D.Vector_Width);
    }
    // Fallback for what used to be HalfVecArgument
    return Tys[D.getArgumentNumber()];
  case IITDescriptor::Pointer: {
    // In LLVM 17+, all pointers are opaque. We just need the AddressSpace.
    // D.Pointer_AddressSpace is the standard field for this descriptor.
    unsigned AS = D.Pointer_AddressSpace;
    return llvm::PointerType::get(Context, AS);
  }
...
static InternalIntrinsic::ID lookupInternalIntrinsicID(StringRef Name) {
  ...
  int Idx = Intrinsic::lookupIntrinsicID(Name);
  if (Idx == -1) {
