...
public:
  ...
  auto &getValue() { return CLOption.value(); }
...
void addExtraPass(PMT &PM, const PassInfo *CurrentPI, PassKind PassKindID,
                  AdderT Adder) {
  auto res = PMOption::option.value().includes(CurrentPI->getPassArgument());
  if (res.first)
...
void addPassImpl(PMT &PM, Pass &P, AdderT Adder) {
  ...
  auto res = DisablePass.value().includes(PassArg);
  if (res.first)
...
void vc::PassManager::add(Pass *P) {
  ...
  if (OverridePassManagerOpt.value())
