...
std::string DumpName::AbsolutePath(OutputFolderName folder) const
{
    ...
    if(m_dumpName.hasValue() && m_dumpName.value() != "")
    {
        ss << m_dumpName.value();
        underscore = true;
    }
    ...
    if (m_cgFlag.hasValue() && m_cgFlag.value() != FLAG_CG_ALL_SIMDS)
    {
        ss << (underscore ? "_" : "");
    ...
    if(m_postfixStr.hasValue() && !m_postfixStr.value().empty())
    {
        ...
        const size_t extSize =
            !m_extension.hasValue() ? 0 : 1 + m_extension.value().size();
