...
NEO::BufferObject *DrmMemoryManager::allocUserptr(uintptr_t address, size_t size, uint32_t rootDeviceIndex) {
    ...
    auto patIndex = drm.getPatIndex(nullptr, AllocationType::externalHostPtr, CacheRegion::defaultRegion, CachePolicy::writeBack, false, true);
...
GraphicsAllocation *DrmMemoryManager::createSharedUnifiedMemoryAllocation(const AllocationData &allocationData) {
    ...
    if (vmAdviseAttribute.hasValue() && vmAdviseAttribute.value() == 0) {
        return nullptr;
    }
    ...
    for (auto handleId = 0u; handleId < numHandles; handleId++) {
        ...
        if (vmAdviseAttribute.hasValue() && !ioctlHelper->setVmBoAdvise(bo->peekHandle(), vmAdviseAttribute.value(), nullptr)) {
            this->munmapFunction(cpuBasePointer, totalSizeToAlloc);
            releaseGpuRange(reinterpret_cast<void *>(preferredAddress), totalSizeToAlloc, allocationData.rootDeviceIndex);
            return nullptr;
        }
