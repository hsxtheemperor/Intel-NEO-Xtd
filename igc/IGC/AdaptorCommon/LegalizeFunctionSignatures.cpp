...
void LegalizeFunctionSignatures::FixFunctionSignatures(Module& M)
{
    ...
    for (auto& FI : M)
    {
        ...
        else if (!isLegalSignatureType(M, pFunc->getReturnType(), isStackCall))
        {
            ...
            argTypes.push_back(IGCLLVM::getPointerType(pFunc->getReturnType(), 0));
        }
        ...
        for (; ai != ei; ai++)
        {
            ...
            {
                fixArgType = true;
                argTypes.push_back(IGCLLVM::getPointerType(ai->getType(), 0));
            }
            else
            {
...
void LegalizeFunctionSignatures::FixCallInstruction(Module& M, CallInst* callInst)
{
    ...
    if (retTypeOption != ReturnOpt::RETURN_DEFAULT || fixArgType)
    {
        ...
        if (!calledFunc)
        {
            ...
            newCalledValue = builder.CreatePointerCast(calledValue, IGCLLVM::getPointerType(newFnTy, 0));
        }
        else
        {
