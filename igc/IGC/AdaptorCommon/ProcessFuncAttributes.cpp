...
static bool isImageType(llvm::Type *Ty)
{
    if (auto *STy = dyn_cast<StructType>(Ty); STy && STy->isOpaque())
    {
        ...
        bool isOpenCLImage = buf[0].equals("opencl") && buf[1].starts_with("image") && buf[1].endswith("_t");
        bool isSPIRVImage = buf[0].equals("spirv") && (buf[1].starts_with("Image") || buf[1].starts_with("SampledImage"));
    ...
    auto NeedsLinking = [](Function* F)
    {
        ...
        if (F->hasExternalLinkage() && F->getCallingConv() == CallingConv::SPIR_FUNC)
        {
            ...
            return !(F->hasFnAttribute("OclBuiltin")
                || F->getName().starts_with("__builtin_")
                || F->getName().starts_with("__igcbuiltin_")
...
#undef _OCL_EXT_OP
    ...
    for (Module::iterator I = M.begin(), E = M.end(); I != E; ++I)
    {
        ...
        if (F->hasFnAttribute("OclBuiltin") ||
            F->getName().starts_with(igc_spv::kLLVMName::builtinPrefix))
        {
