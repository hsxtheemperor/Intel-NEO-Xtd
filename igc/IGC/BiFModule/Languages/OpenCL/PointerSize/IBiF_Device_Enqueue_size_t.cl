    ...
    clk_event_t event)
{
    ...
    if( offset >= 0 )
    {
        ...
        pCommand->m_event                 = (int)(size_t)intel_bridge_to_handle(event);
        ...
        if( 0 != (size_t)intel_bridge_to_handle(event) )
        {
            __global IGIL_DeviceEvent *events = IGIL_GetDeviceEvents();

            atomic_inc( &events[(int)(size_t)intel_bridge_to_handle(event)].m_numChildren );
        }
    ...
    clk_event_t parentEvent)
{
    ...
    if( 0 != out_event )
    {
        myEvent = intel_bridge_from_handle_clk_event((uintptr_t)IGIL_AcquireEvent());

        if( 0 == (size_t)intel_bridge_to_handle(myEvent) )
        {
          status = CLK_EVENT_ALLOCATION_FAILURE;
        }
        else if( 0 != (size_t)intel_bridge_to_handle(parentEvent) )
        {
            // if parent event, update its count of children to include newly created event

            events[(int)(size_t)intel_bridge_to_handle(myEvent)].m_parentEvent = (size_t)intel_bridge_to_handle(parentEvent);
            atomic_inc( &events[(int)(size_t)intel_bridge_to_handle(parentEvent)].m_numChildren );
            events[(int)(size_t)intel_bridge_to_handle(myEvent)].m_state = CL_QUEUED;
        }
    }
    ...
    else
    {
        ...
        if( pCommand != 0 )
        {
            ...
            for( int i = 0; i < numEvents; i++ )
            {
                ...
                pCommand->m_data[i] = (unsigned)(intel_bridge_to_handle(d));
                atomic_inc( &events[(int)(size_t)intel_bridge_to_handle(d)].m_numDependents );
            }
        }
        else
        {
            ...
            if( NULL != out_event )
            {
                ...
                myEvent = intel_bridge_from_handle_clk_event(0);
            }
        }
    }
...

{
    return IGIL_EnqueueKernel( q, block_id,
                               ...
                               getobjectidMappingBuf, numArgMappings,
                               range, flags,
                               intel_bridge_from_handle_clk_event((uintptr_t)__builtin_IB_get_parent_event()) 
                               );
}
