...
#include "WrapperLLVM/include/llvmWrapper/IR/Instructions.h"
#include "WrapperLLVM/include/llvmWrapper/IR/Intrinsics.h"
...
static ::llvm::ConstantAsMetadata *getConstMdOperand(const ::llvm::MDNode *MD, unsigned OpIdx) {
  return ::llvm::cast<::llvm::ConstantAsMetadata>(MD->getOperand(OpIdx));
}
static const ::llvm::APInt &getIntConstFromMdOperand(const ::llvm::MDNode *MD, unsigned OpIdx) {
...
PieceBuilder::PieceInfo PieceBuilder::get(unsigned index) const {
...
CompileUnit::CompileUnit(unsigned UID, DIE *D, ::llvm::DICompileUnit *Node,
                         StreamEmitter *A, DwarfDebug *DW)
...
/// information entry.
DIEEntry *CompileUnit::createDIEEntry(class DIE *Entry) {
  DIEEntry *Value =
...
static bool isShareableAcrossCUs(const ::llvm::MDNode *D) {
  return (::llvm::isa<::llvm::DIType>(D) ||
          (::llvm::isa<::llvm::DISubprogram>(D) && !(::llvm::cast<::llvm::DISubprogram>(D)->isDefinition())));
}

/// getDIE - Returns the debug information entry map slot for the
/// specified debug variable. We delegate the request to DwarfDebug
/// when the class DIE for this ::llvm::MDNode can be shared across CUs. The mappings
/// will be kept in DwarfDebug for shareable DIEs.
class DIE *CompileUnit::getDIE(::llvm::DINode *D) const {
...
/// when the class DIE for this ::llvm::MDNode can be shared across CUs. The mappings
/// will be kept in DwarfDebug for shareable DIEs.
void CompileUnit::insertDIE(::llvm::MDNode *Desc, class DIE *D) {
...
void CompileUnit::addFlag(class DIE *Die, dwarf::Attribute Attribute) {
...
///
void CompileUnit::addUInt(class DIE *Die, dwarf::Attribute Attribute,
...
void CompileUnit::addUInt(DIEBlock *Block, dwarf::Form Form,
                          uint64_t Integer) {
  IGC_ASSERT_MESSAGE(
...
void CompileUnit::addBitPiece(DIEBlock *Block, uint64_t SizeBits,
                              uint64_t OffsetBits) {
...
///
void CompileUnit::addSInt(class DIE *Die, dwarf::Attribute Attribute,
...
void CompileUnit::addSInt(DIEBlock *Die, std::optional<dwarf::Form> Form,
                          int64_t Integer) {
...
/// table.
void CompileUnit::addString(class DIE *Die, dwarf::Attribute Attribute,
                            StringRef String) {
  // Emit string inlined
...
///
void CompileUnit::addExpr(DIEBlock *Die, dwarf::Form Form,
                          const MCExpr *Expr) {
...
///
void CompileUnit::addLabel(class DIE *Die, dwarf::Attribute Attribute,
...
///
void CompileUnit::addLabelAddress(class DIE *Die, dwarf::Attribute Attribute,
                                  MCSymbol *Label) {
  if (Label) {
...
void CompileUnit::addLabelLoc(class DIE *Die, dwarf::Attribute Attribute,
                              MCSymbol *Label) {
  if (Label != NULL) {
    DD->addArangeLabel(SymbolCU(this, Label));
    DIEValue *Value = new (DIEValueAllocator) DIELabel(Label);
...
///
void CompileUnit::addDelta(class DIE *Die, dwarf::Attribute Attribute,
...
///
void CompileUnit::addDIEEntry(class DIE *Die, dwarf::Attribute Attribute,
                              class DIE *Entry) {
...
void CompileUnit::addDIEEntry(class DIE *Die, dwarf::Attribute Attribute,
                              IGC::DIEEntry *Entry) {
  const class DIE *DieCU = Die->getCompileUnitOrNull();
  const class DIE *EntryCU = Entry->getEntry()->getCompileUnitOrNull();
  if (!DieCU) {
...
class DIE *CompileUnit::createAndAddDIE(unsigned Tag, class DIE &Parent,
                                       ::llvm::DINode *N) {
  class DIE *Die = new class DIE(Tag);
  Parent.addChild(Die);
  if (N) {
    insertDIE(N, Die);
  }
...
///
void CompileUnit::addBlock(class DIE *Die, dwarf::Attribute Attribute,
                           DIEBlock *Block) {
...
/// entry.
void CompileUnit::addSourceLine(class DIE *Die, DIScope *S, unsigned Line) {
...
/// addSourceLine - Add location information to specified debug information
/// entry.
void CompileUnit::addSourceLine(class DIE *Die, DIImportedEntity *IE, unsigned Line) {
  // If the line number is 0, don't add it.
  if (Line == 0)
    return;
...
/// addSourceLine - Add location information to specified debug information
/// entry.
void CompileUnit::addSourceLine(class DIE *Die, DIVariable *V) {
  // Verify variable.
  if (!::llvm::isa<DIVariable>(V))
    return;
...
/// addSourceLine - Add location information to specified debug information
/// entry.
void CompileUnit::addSourceLine(class DIE *Die, llvm::DISubprogram *SP) {
  // Verify subprogram.
  if (!::llvm::isa<llvm::DISubprogram>(SP))
    return;
...
/// addSourceLine - Add location information to specified debug information
/// entry.
void CompileUnit::addSourceLine(class DIE *Die, llvm::DIType *Ty) {
  // Verify type.
  if (!::llvm::isa<llvm::DIType>(Ty))
    return;
...
static bool isTypeSigned(DwarfDebug *DD, llvm::DIType *Ty, int *SizeInBits) {
  if (::llvm::isa<DIDerivedType>(Ty)) {
    return isTypeSigned(DD, DD->resolve(::llvm::cast<DIDerivedType>(Ty)->getBaseType()),
                        SizeInBits);
  }
  if (::llvm::isa<DIBasicType>(Ty)) {
    if (::llvm::cast<DIBasicType>(Ty)->getEncoding() == dwarf::DW_ATE_signed ||
        ::llvm::cast<DIBasicType>(Ty)->getEncoding() == dwarf::DW_ATE_signed_char) {
      *SizeInBits = (int)::llvm::cast<DIBasicType>(Ty)->getSizeInBits();
      return true;
    }
  }
  return false;
...
bool isUnsignedDIType(DwarfDebug *DD, llvm::DIType *Ty) {
...
void CompileUnit::addConstantFPValue(class DIE *Die, const ConstantFP *CFP) {
  // Pass this down to addConstantValue as an unsigned bag of bits.
  addConstantValue(Die, CFP->getValueAPF().bitcastToAPInt(), true);
}

/// addConstantValue - Add constant value entry in variable class DIE.
void CompileUnit::addConstantValue(class DIE *Die, const ConstantInt *CI,
                                   bool Unsigned) {
  addConstantValue(Die, CI->getValue(), Unsigned);
}

// addConstantValue - Add constant value entry in variable class DIE.
void CompileUnit::addConstantValue(class DIE *Die, const APInt &Val, bool Unsigned) {
...
void CompileUnit::addConstantUValue(IGC::DIEBlock *TheDie, uint64_t Val) {
  if (Val <= 31) {
...
void CompileUnit::addConstantData(class DIE *Die, const unsigned char *Ptr8,
                                  int NumBytes) {
...
void CompileUnit::addTemplateParams(class DIE &Buffer, ::llvm::DINodeArray TParams) {
  // Add template parameters.
  for (const auto *Element : TParams) {
    if (auto *TTP = ::llvm::dyn_cast<DITemplateTypeParameter>(Element))
      constructTemplateTypeParameterDIE(
          Buffer, const_cast<DITemplateTypeParameter *>(TTP));
    else if (auto *TVP = ::llvm::dyn_cast<DITemplateValueParameter>(Element))
...
class DIE *CompileUnit::getOrCreateContextDIE(DIScope *Context) {
  if (!Context || ::llvm::isa<DIFile>(Context))
    return getCUDie();
  if (auto *T = ::llvm::dyn_cast<llvm::DIType>(Context))
    return getOrCreateTypeDIE(T);
  if (auto *NS = ::llvm::dyn_cast<DINamespace>(Context))
    return getOrCreateNameSpace(NS);
  if (auto *SP = ::llvm::dyn_cast<llvm::DISubprogram>(Context))
    return getOrCreateSubprogramDIE(SP);
  if (auto *MD = ::llvm::dyn_cast<DIModule>(Context))
...
/// given ::llvm::DIType.
class DIE *CompileUnit::getOrCreateTypeDIE(const ::llvm::MDNode *TyNode) {
  if (!TyNode)
    return NULL;

  ::llvm::DIType *const Ty = ::llvm::cast_or_null<::llvm::DIType>(const_cast<::llvm::MDNode *>(TyNode));
  IGC_ASSERT(nullptr != Ty);

  // Construct the context before querying for the existence of the class DIE in case
  // such construction creates the class DIE.
  class DIE *const ContextDIE = getOrCreateContextDIE(resolve(Ty->getScope()));
  IGC_ASSERT(nullptr != ContextDIE);

  class DIE *TyDIE = getDIE(Ty);
  if (nullptr != TyDIE)
    return TyDIE;
  ...
  if (::llvm::isa<::llvm::DIBasicType>(Ty))
    constructTypeDIE(*TyDIE, ::llvm::cast<::llvm::DIBasicType>(Ty));
  else if (::llvm::isa<::llvm::DICompositeType>(Ty))
    constructTypeDIE(*TyDIE, ::llvm::cast<::llvm::DICompositeType>(Ty));
  else if (::llvm::isa<::llvm::DISubroutineType>(Ty))
    constructTypeDIE(*TyDIE, ::llvm::cast<::llvm::DISubroutineType>(Ty));
  else {
    IGC_ASSERT_MESSAGE(::llvm::isa<::llvm::DIDerivedType>(Ty), "Unknown kind of ::llvm::DIType");
    constructTypeDIE(*TyDIE, ::llvm::cast<::llvm::DIDerivedType>(Ty));
  }
...
void CompileUnit::addType(class DIE *Entity, ::llvm::DIType *Ty, dwarf::Attribute Attribute) {
  ...
  // Construct type.
  class DIE *Buffer = getOrCreateTypeDIE(Ty);
...
void CompileUnit::addSimdWidth(class DIE *Die, uint16_t SimdWidth) {
  // Emit SIMD width
  addUInt(Die, (dwarf::Attribute)DW_AT_INTEL_simd_width, dwarf::DW_FORM_data2,
          SimdWidth);
}

void CompileUnit::extractSubRegValue(DIEBlock *Block, unsigned char Sz) {
...
void CompileUnit::addBindlessOrStatelessLocation(
    DIEBlock *Block, const VISAVariableLocation &Loc,
...
void CompileUnit::addStatelessLocation(DIEBlock *Block,
...
void CompileUnit::addBindlessSurfaceLocation(DIEBlock *Block,
                                             const VISAVariableLocation &Loc) {
...
void CompileUnit::addBindlessSamplerLocation(DIEBlock *Block,
                                             const VISAVariableLocation &Loc) {
...
void CompileUnit::addBE_FP(DIEBlock *Block) {
...
void CompileUnit::addScratchLocation(DIEBlock *Block,
...
// 12 DW_OP_INTEL_bit_piece_stack
//
void CompileUnit::addSimdLane(DIEBlock *Block, const DbgVariable &DV,
                              const VISAVariableLocation &Loc,
  ...
  if (lr->isSpill()) {
    ...
    uint32_t valForSubRegLit =
        variablesInSingleGRF > 0 ? (uint32_t)std::log2(variablesInSingleGRF) : 0;
...
void CompileUnit::emitBitPiecesForRegVal(DIEBlock *Block,
...
void CompileUnit::addSimdLaneScalar(DIEBlock *Block, const DbgVariable &DV,
                                    const VISAVariableLocation &Loc,
                                    const DbgDecoder::LiveIntervalsVISA &lr) {
...
void CompileUnit::addSimdLaneRegionBase(
    DIEBlock *Block, const DbgVariable &DV,
...
std::string CompileUnit::getParentContextString(DIScope *Context) const {
  ...
  while (!::llvm::isa<::llvm::DICompileUnit>(Context)) {
...
void CompileUnit::constructTypeDIE(class DIE &Buffer, DIBasicType *BTy) {
...
void CompileUnit::constructTypeDIE(class DIE &Buffer, DIDerivedType *DTy) {
  // Get core information.
  ...
  ::llvm::DIType *FromTy = resolve(DTy->getBaseType());
  if (FromTy)
  ...
  if (DTy->getDWARFAddressSpace().has_value() &&
      isSLMAddressSpaceTag(DTy->getDWARFAddressSpace().value())) {
...
/// its own type unit.
static bool isTypeUnitScoped(::llvm::DIType *Ty, const DwarfDebug *DD) {
  DIScope *Parent = DD->resolve(Ty->getScope());
  while (Parent) {
    // Don't generate a hash for anything scoped inside a function.
    if (::llvm::isa<::llvm::DISubprogram>(Parent))
      return false;
...
void CompileUnit::constructTypeDIE(class DIE &Buffer, DISubroutineType *STy) {
  DITypeRefArray Elements = ::llvm::cast<DISubroutineType>(STy)->getTypeArray();
  ::llvm::DIType *RTy = resolve(Elements[0]);
  if (RTy)
    addType(&Buffer, RTy);
  ...
  for (unsigned i = 1, N = Elements.size(); i < N; ++i) {
    ::llvm::DIType *Ty = resolve(Elements[i]);
    if (!Ty) {
      createAndAddDIE(dwarf::DW_TAG_unspecified_parameters, Buffer);
      isPrototyped = false;
    } else {
      class DIE *Arg = createAndAddDIE(dwarf::DW_TAG_formal_parameter, Buffer);
      addType(Arg, Ty);
      if (Ty->isArtificial())
...
/// constructTypeDIE - Construct type class DIE from DICompositeType.
void CompileUnit::constructTypeDIE(class DIE &Buffer, DICompositeType *CTy) {
  // Get core information.
  ...
  case dwarf::DW_TAG_subroutine_type: {
    ...
    DITypeRefArray Elements = ::llvm::cast<DISubroutineType>(CTy)->getTypeArray();
    ::llvm::DIType *RTy = resolve(Elements[0]);
    if (RTy)
      addType(&Buffer, RTy);
  ...
  case dwarf::DW_TAG_class_type: {
    ...
    for (unsigned i = 0, N = Elements.size(); i < N; ++i) {
      ::llvm::DINode *Element = Elements[i];
      class DIE *ElemDie = NULL;
      if (::llvm::isa<::llvm::DISubprogram>(Element)) {
        ::llvm::DISubprogram *SP = ::llvm::cast<::llvm::DISubprogram>(Element);
      ...
      } else if (::llvm::isa<DIDerivedType>(Element)) {
        DIDerivedType *DDTy = ::llvm::cast<DIDerivedType>(Element);
    ...
    ::llvm::DIType *ContainingType = resolve(CTy->getBaseType());
    if (ContainingType && ::llvm::isa<DICompositeType>(ContainingType))
      addDIEEntry(&Buffer, dwarf::DW_AT_containing_type,
...
void CompileUnit::constructTemplateTypeParameterDIE(
    class DIE &Buffer, DITemplateTypeParameter *TP) {
  class DIE *ParamDIE =
...
void CompileUnit::constructTemplateValueParameterDIE(
    class DIE &Buffer, DITemplateValueParameter *VP) {
  class DIE *ParamDIE = createAndAddDIE(VP->getTag(), Buffer);
  ...
  if (Metadata *Val = VP->getValue()) {
    if (ConstantInt *CI = mdconst::dyn_extract<ConstantInt>(Val)) {
      ...
      IGC_ASSERT(::llvm::isa<MDString>(Val));
      addString(ParamDIE, dwarf::DW_AT_GNU_template_name,
                ::llvm::cast<MDString>(Val)->getString());
    } else if (VP->getTag() == dwarf::DW_TAG_GNU_template_parameter_pack) {
      IGC_ASSERT(::llvm::isa<::llvm::MDNode>(Val));
      // DIArray A(::llvm::cast<::llvm::MDNode>(Val));
      // addTemplateParams(*ParamDIE, A);
    }
  }
}

/// constructImportedEntityDIE - Create a class DIE for DIImportedEntity.
class DIE *CompileUnit::constructImportedEntityDIE(DIImportedEntity *Module) {
  ::llvm::DINode *Entity = Module->getEntity();
  if (auto *GV = ::llvm::dyn_cast<DIGlobalVariable>(Entity))
    return nullptr; // Missing support for imported entity linked to a global
                    // variable

  class DIE *IMDie = new class DIE(Module->getTag());
  insertDIE(Module, IMDie);

  class DIE *EntityDie;
  if (auto *NS = ::llvm::dyn_cast<DINamespace>(Entity))
    EntityDie = getOrCreateNameSpace(NS);
  else if (auto *M = ::llvm::dyn_cast<DIModule>(Entity))
    EntityDie = getOrCreateModuleDIE(M);
  else if (auto *SP = ::llvm::dyn_cast<::llvm::DISubprogram>(Entity))
    EntityDie = getOrCreateSubprogramDIE(SP);
  else if (auto *T = ::llvm::dyn_cast<::llvm::DIType>(Entity))
    EntityDie = getOrCreateTypeDIE(T);
  // else if (auto* GV = ::llvm::dyn_cast<DIGlobalVariable>(Entity))  // TODO missing
  // support
...
class DIE *CompileUnit::getOrCreateNameSpace(DINamespace *NS) {
  // Construct the context before querying for the existence of the class DIE in case
  // such construction creates the class DIE.
  class DIE *ContextDIE = getOrCreateContextDIE(NS->getScope());

  class DIE *NDie = getDIE(NS);
  if (NDie)
    return NDie;
...
class DIE *CompileUnit::getOrCreateSubprogramDIE(::llvm::DISubprogram *SP) {
  // Construct the context before querying for the existence of the class DIE in case
  // such construction creates the class DIE (as is the case for member function
  // declarations).
  class DIE *ContextDIE = getOrCreateContextDIE(resolve(SP->getScope()));

  class DIE *SPDie = getDIE(SP);
  if (SPDie)
    return SPDie;

  ::llvm::DISubprogram *SPDecl = SP->getDeclaration();
  if (SPDecl && ::llvm::isa<::llvm::DISubprogram>(SPDecl)) {
    // Add subprogram definitions to the CU die directly.
    ContextDIE = CUDie;
  }

  // DW_TAG_inlined_subroutine may refer to this class DIE.
  SPDie = createAndAddDIE(dwarf::DW_TAG_subprogram, *ContextDIE, SP);

  class DIE *DeclDie = NULL;
  if (SPDecl && ::llvm::isa<::llvm::DISubprogram>(SPDecl)) {
  ...
  // If this class DIE is going to refer declaration info using AT_specification
  ...
  if (!SP->isDefinition()) {
    ...
    for (unsigned i = 1, N = Args.size(); i < N; ++i) {
      class DIE *Arg = createAndAddDIE(dwarf::DW_TAG_formal_parameter, *SPDie);
      ::llvm::DIType *ATy = resolve(Args[i]);
      if (!ATy)
        continue;
      addType(Arg, ATy);
      if (ATy->isArtificial()) {
...
class DIE *CompileUnit::getOrCreateModuleDIE(DIModule *MD) {
  // Construct the context before querying for the existence of the class DIE in case
  // such construction creates the class DIE (as is the case for member function
  // declarations).
  class DIE *ContextDIE = getOrCreateContextDIE(MD->getScope());
  class DIE *MDDie = getDIE(MD);
  if (MDDie)
    return MDDie;
...
/// constructSubrangeDIE - Construct subrange class DIE from DISubrange.
void CompileUnit::constructSubrangeDIE(class DIE &Buffer, DISubrange *SR,
                                       class DIE *IndexTy) {
  class DIE *DW_Subrange = createAndAddDIE(dwarf::DW_TAG_subrange_type, Buffer);
  addDIEEntry(DW_Subrange, dwarf::DW_AT_type, IndexTy);

  int64_t DefaultLowerBound = getDefaultLowerBound();

#if LLVM_VERSION_MAJOR >= 13
  // Define the lambda inside the version-guarded block where BoundType exists
  auto AddBoundTypeEntry = [&](::llvm::dwarf::Attribute Attr,
                               ::llvm::DISubrange::BoundType Bound) {
    if (auto *BV = ::llvm::dyn_cast<::llvm::DIVariable *>(Bound)) {
      if (auto *VarDIE = getDIE(BV))
        addDIEEntry(DW_Subrange, Attr, VarDIE);
    } else if (auto *BE = ::llvm::dyn_cast<::llvm::DIExpression *>(Bound)) {
      class DIEBlock *Loc = new (DIEValueAllocator) class DIEBlock;
      DIEDwarfExpression DwarfExpr(*Asm, getCU(), *Loc);
      DwarfExpr.addExpression(BE);
      addBlock(DW_Subrange, Attr, DwarfExpr.finalize());
    } else if (auto *BI = ::llvm::dyn_cast<::llvm::ConstantInt *>(Bound)) {
      if (Attr == ::llvm::dwarf::DW_AT_count) {
        if (BI->getSExtValue() != -1)
          addUInt(DW_Subrange, Attr, std::nullopt, BI->getSExtValue());
      } else if (Attr != ::llvm::dwarf::DW_AT_lower_bound || DefaultLowerBound == -1 ||
                 BI->getSExtValue() != DefaultLowerBound) {
        addSInt(DW_Subrange, Attr, ::llvm::dwarf::DW_FORM_sdata, BI->getSExtValue());
      }
    }
  };
...
#else
  // Legacy path for LLVM 12 and below
  int64_t LowerBound = SR->getLowerBound();
  
  // Fixed the dyn_cast syntax here as well
  auto *CI = ::llvm::dyn_cast<::llvm::ConstantInt *>(SR->getCount());
...
/// constructArrayTypeDIE - Construct array type class DIE from DICompositeType.
void CompileUnit::constructArrayTypeDIE(class DIE &Buffer, DICompositeType *CTy) {
  if (CTy->isVector()) {
  ...
  for (unsigned i = 0, N = Elements.size(); i < N; ++i) {
    ...
    if (Element->getTag() == dwarf::DW_TAG_subrange_type) {
      constructSubrangeDIE(Buffer, ::llvm::cast<DISubrange>(Element), IdxTy);
    }
  }
}

/// constructEnumTypeDIE - Construct an enum type class DIE from DICompositeType.
void CompileUnit::constructEnumTypeDIE(class DIE &Buffer, DICompositeType *CTy) {
  DINodeArray Elements = CTy->getElements();
  ...
  for (unsigned i = 0, N = Elements.size(); i < N; ++i) {
    ...
    if (Enum) {
      class DIE *Enumerator = createAndAddDIE(dwarf::DW_TAG_enumerator, Buffer);
  ...
  ::llvm::DIType *DTy = resolve(CTy->getBaseType());
  if (DTy) {
    addType(&Buffer, DTy);
...
void CompileUnit::constructContainingTypeDIEs() {
  for (DenseMap<class DIE *, const ::llvm::MDNode *>::iterator CI = ContainingTypeMap.begin(),
                                                 CE = ContainingTypeMap.end();
       CI != CE; ++CI) {
    class DIE *SPDie = CI->first;
    ::llvm::DINode *D = ::llvm::cast<::llvm::DINode>(const_cast<::llvm::MDNode *>(CI->second));
    if (!D)
      continue;
    class DIE *NDie = getDIE(D);
    if (!NDie)
      continue;
...
class DIE *CompileUnit::constructVariableDIE(DbgVariable &DV,
                                            bool isScopeAbstract) {
  StringRef Name = DV.getName();
  LLVM_DEBUG(dbgs() << "[DwarfDebug] constructing class DIE for variable <" << Name
                    << ">\n");
  // Define variable debug information entry.
  class DIE *VariableDie = new class DIE(DV.getTag());
  DbgVariable *AbsVar = DV.getAbstractVariable();
  class DIE *AbsDIE = AbsVar ? AbsVar->getDIE() : NULL;
  if (AbsDIE) {
  ...
  LLVM_DEBUG(dbgs() << " done. Location is emitted directly in class DIE\n");
  return VariableDie;
}

void CompileUnit::buildLocation(const llvm::Instruction *pDbgInst,
                                DbgVariable &DV, class DIE *VariableDie) {
  ...
  if (Loc.IsImmediate()) {
    ...
    if (const ConstantInt *pConstInt = ::llvm::dyn_cast<ConstantInt>(pConstVal)) {
      addConstantValue(VariableDie, pConstInt,
                       isUnsignedDIType(DD, DV.getType()));
    } else if (const ConstantFP *pConstFP = ::llvm::dyn_cast<ConstantFP>(pConstVal)) {
...
DIEBlock *CompileUnit::buildPointer(const DbgVariable &var,
                                         const VISAVariableLocation &loc) {
...
DIEBlock *CompileUnit::buildSampler(const DbgVariable &var,
                                         const VISAVariableLocation &loc) {
  DIEBlock *Block = new (DIEValueAllocator) DIEBlock();
...
DIEBlock *CompileUnit::buildSLM(const DbgVariable &var,
                                     const VISAVariableLocation &loc,
                                     class DIE *VariableDie) {
  ...
  IGC_ASSERT_MESSAGE(VariableDie,
                     "Expected variable class DIE to emit address_class for SLM");
...
bool CompileUnit::buildFpBasedLoc(const DbgVariable &var, DIEBlock *Block,
                                  const VISAVariableLocation &loc) {
  const auto *storageMD = var.getDbgInst()->getMetadata("StorageOffset");
...
bool CompileUnit::buildSlicedLoc(
    const DbgVariable &var, DIEBlock *Block,
    const VISAVariableLocation &loc,
  ...
  IGC_ASSERT_MESSAGE(::llvm::isa<DIEInteger>(secondHalfOff), "Expecting DIEInteger");
  ...
  IGC_ASSERT_MESSAGE(::llvm::isa<DIEInteger>(skipOff), "Expecting DIEInteger");
  offsetTaken = Block->ComputeSizeOnTheFly(Asm);
  ::llvm::cast<DIEInteger>(secondHalfOff)->setValue(offsetTaken - offsetNotTaken);
...
DIEBlock *CompileUnit::buildGeneral(
  ...
  if (skipOff) {
    ...
    ::llvm::cast<DIEInteger>(skipOff)->setValue(offsetEnd - offsetTaken);
  }
...
/// constructMemberDIE - Construct member class DIE from DIDerivedType.
void CompileUnit::constructMemberDIE(class DIE &Buffer, DIDerivedType *DT) {
  class DIE *MemberDie = createAndAddDIE(DT->getTag(), Buffer);
  StringRef Name = DT->getName();
  if (!Name.empty()) {
    addString(MemberDie, dwarf::DW_AT_name, Name);
  }

  ::llvm::DIType *BaseTy = resolve(DT->getBaseType());
...
/// getOrCreateStaticMemberDIE - Create new class DIE for C++ static member.
class DIE *CompileUnit::getOrCreateStaticMemberDIE(DIDerivedType *DT) {
  // Construct the context before querying for the existence of the class DIE in case
  // such construction creates the class DIE.
  class DIE *ContextDIE = getOrCreateContextDIE(resolve(DT->getScope()));
  IGC_ASSERT_MESSAGE(dwarf::isType(ContextDIE->getTag()),
                     "Static member should belong to a type.");

  class DIE *StaticMemberDIE = getDIE(DT);
  if (StaticMemberDIE)
    return StaticMemberDIE;

  StaticMemberDIE = createAndAddDIE(DT->getTag(), *ContextDIE, DT);

  ::llvm::DIType *Ty = resolve(DT->getBaseType());
