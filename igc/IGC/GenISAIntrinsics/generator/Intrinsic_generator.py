...
from igc.IGC.GenISAIntrinsics.generator.Intrinsic_definition_translation import generate_intrinsic_definitions_from_modules
...
class IntrinsicFormatter:
    ...
    def get_attribute_entry(attribute, is_last):
        _attr_map = {
            "ArgMemOnly": "Memory",
            "InaccessibleMemOnly": "Memory",
            "InaccessibleMemOrArgMemOnly": "Memory",
            "ReadNone": "Memory",
            "ReadOnly": "Memory",
            "WriteOnly": "Memory",
        }
        mapped = _attr_map.get(str(attribute), str(attribute))
        output = 'llvm::Attribute::{}'.format(mapped)
        if not is_last:
            output = "{},".format(output)
        return output
