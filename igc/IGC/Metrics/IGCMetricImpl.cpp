...
namespace IGCMetrics
{
    ...
    public:
        ...
        void visitCallInst(llvm::CallInst& callInst)
        {
            ...
            if (calledFuncName.starts_with("llvm.dbg") ||
                calledFuncName.starts_with("llvm.genx.GenISA.CatchAllDebugLine"))
            {
            ...
            if (calledFuncName.starts_with("__builtin_IB"))
            {
                funcCallType = IGC_METRICS::FuncCalls_FuncCallsType::FuncCalls_FuncCallsType_LIBRARY;
            }
            else if (calledFuncName.starts_with("llvm."))
            {
                funcCallType = IGC_METRICS::FuncCalls_FuncCallsType::FuncCalls_FuncCallsType_LIBRARY;
            }
            else if (calledFuncName.starts_with("__builtin_spirv"))
            {
                funcCallType = IGC_METRICS::FuncCalls_FuncCallsType::FuncCalls_FuncCallsType_LIBRARY;
            }
    ...
    void IGCMetricImpl::GetFunctionData(IGC_METRICS::Function* func_m, llvm::Function& func)
    {
        ...
        for (auto bb_i = func.begin(); bb_i != func.end(); ++bb_i)
        {
            ...
            if (bb->hasName() && (bb->getName().starts_with("if.then") ||
                bb->getName().starts_with("if.else")))
            {
    ...
    struct VarData* IGCMetricImpl::GetVarData(llvm::Value* pValue)
    {
            ...
            ++trackerVal_i)
        {
            ...
            for (auto user : tracker->users())
            {
                ...
                {
                    if (callInst->getCalledFunction()->getName().starts_with(funcTrackValue))
                    {
