...
static Triple overrideTripleWithVC(StringRef TripleStr) {
  ...
  if (TripleStr.starts_with("genx32"))
    Is32Bit = true;
...
static GenXBackendOptions createBackendOptions(const vc::CompileOptions &Opts) {
  ...
  if (Opts.StackMemSize) {
    BackendOpts.StackSurfaceMaxSize = Opts.StackMemSize.value();
    BackendOpts.StatelessPrivateMemSize = Opts.StackMemSize.value();
  }
  ...
  if (Opts.GRFSize)
    BackendOpts.GRFSize = Opts.GRFSize.value();
