    ...
    Instruction *InsertBefore)
{
  ...
  Function *Decl = GenXIntrinsic::getGenXDeclaration(M,
      ...
      OverloadedTypes);
  auto WrRegion = CallInst::Create(Decl, Args, Name, InsertBefore->getIterator());
...
void CMSimdCFLower::rewritePredication(CallInst *CI, unsigned SimdWidth)
{
  ...
  auto Select = SelectInst::Create(EM, EnabledValues, DisabledDefaults,
      EnabledValues->getName() + ".simdcfpred", CI->getIterator());
...
void CMSimdCFLower::predicateStore(Instruction *SI, unsigned SimdWidth)
{
  ...
  auto Select = SelectInst::Create(EM, SI->getOperand(0), Load,
      SI->getOperand(0)->getName() + ".simdcfpred", SI->getIterator());
...
CallInst *CMSimdCFLower::predicateWrRegion(CallInst *WrR, unsigned SimdWidth)
{
  ...
  if (!Pred)
    ...
    auto And = BinaryOperator::Create(Instruction::And, EM, Pred,
        Pred->getName() + ".and." + EM->getName(), WrR->getIterator());
...
void CMSimdCFLower::lowerSimdCF()
{
  ...
  for (auto sbi = SimdBranches.begin(), sbe = SimdBranches.end();
    ...
    auto OldRM =
        new LoadInst(cast<AllocaInst>(RMAddr)->getAllocatedType(), RMAddr,
                     RMAddr->getName(), false /* isVolatile */, Br->getIterator());
    ...
    Goto->setDebugLoc(DL);
    Goto->setConvergent();
    Instruction *NewEM = ExtractValueInst::Create(Goto, 0, "goto.extractem", Br->getIterator());
    (new StoreInst(NewEM, EMVar, false /* isVolatile */, Br->getIterator()))->setDebugLoc(DL);
    auto NewRM = ExtractValueInst::Create(Goto, 1, "goto.extractrm", Br->getIterator());
    (new StoreInst(NewRM, RMAddr, false /* isVolatile */, Br->getIterator()))->setDebugLoc(DL);
    auto BranchCond = ExtractValueInst::Create(Goto, 2, "goto.extractcond", Br->getIterator());
  ...
  for (auto jpi = JoinPoints.begin(), jpe = JoinPoints.end();
    ...
    BasicBlock::iterator InsertBefore = JP->getFirstNonPHIIt();
    ...
    auto RM =
        new LoadInst(cast<AllocaInst>(RMAddr)->getAllocatedType(), RMAddr,
                     RMAddr->getName(), false /* isVolatile */, InsertBefore->getIterator());
    RM->setDebugLoc(DL);
    ...
    Value *Args[] = { OldEM, RM };
    auto Join = CallInst::Create(JoinFunc, Args, "join", InsertBefore->getIterator());
    Join->setDebugLoc(DL);
    Join->setConvergent();
    auto NewEM = ExtractValueInst::Create(Join, 0, "join.extractem", InsertBefore->getIterator());
    (new StoreInst(NewEM, EMVar, false /* isVolatile */, InsertBefore->getIterator()))
        ->setDebugLoc(DL);
    auto BranchCond = ExtractValueInst::Create(Join, 1, "join.extractcond", InsertBefore->getIterator());
    // Zero RM.
    ...
    if (JIP) {
      ...
      auto NewBr = BranchInst::Create(JIP, JP->getNextNode(), BranchCond, Br->getIterator());
    ...
    for (auto bi = BB->begin(), be = BB->end(); bi != be; ++bi) {
      ...
      if (auto *CIE = dyn_cast<CallInst>(Inst)) {
        if (GenXIntrinsic::getGenXIntrinsicID(CIE) ==
          ...
          Value *Args[] = {OldEM};
          auto Savemask = CallInst::Create(SavemaskFunc, Args, "savemask", CIB->getIterator());
          ...
          auto Unmask = CallInst::Create(UnmaskFunc, Arg1s, "unmask", CIB->getIterator());
          Unmask->setDebugLoc(DL);
          (new StoreInst(Unmask, EMVar, false /* isVolatile */, CIB->getIterator()))
              ->setDebugLoc(DL);
          ...
          auto Remask = CallInst::Create(RemaskFunc, Arg2s, "remask", CIE->getIterator());
          Remask->setDebugLoc(DLCIE);
          (new StoreInst(Remask, EMVar, false /* isVolatile */, CIE->getIterator()))
              ->setDebugLoc(DLCIE);
...
Instruction *CMSimdCFLower::loadExecutionMask(Instruction *InsertBefore,
  ...
  EM = new ShuffleVectorInst(EM, UndefValue::get(EM->getType()),
                             ...
                             Twine("EM") + Twine(SimdWidth), InsertBefore->getIterator());
...
Value *CMSimdCFLower::getRMAddr(BasicBlock *JP, unsigned SimdWidth)
{
  ...
  if (!*RMAddr) {
    ...
    *RMAddr = new AllocaInst(RMTy, /*AddrSpace*/ 0,
                             Twine("RM.") + JP->getName(), InsertBefore->getIterator());
