...
Type *SEVUtil::getTypeFreeFromSEV(Type *Ty) {
  ...
  if (auto *Ptr = dyn_cast<PointerType>(Ty)) {
    ...
    if (UTy == VCINTR::Type::getNonOpaquePtrEltTy(Ptr))
      return Ptr;
    return llvm::PointerType::get(UTy->getContext(), Ptr->getAddressSpace());;
...
void SEVUtil::replaceAllUsesWith(Function &OldF, Function &NewF) {
  ...
  for (auto *U : Users) {
    ...
    auto *NewCall = CallInst::Create(&NewF, NewParams, "", OldInst->getIterator());
...
#if VC_INTR_LLVM_VERSION_MAJOR >= 18 || LLVM_VERSION_MAJOR >= 18
...
Instruction *SEVUtil::visitStoreInst(StoreInst &OldInst) {
  ...
  auto *NewStore = new llvm::StoreInst(NewVals[0], NewVals[1], OldInst.isVolatile(),
    llvm::Align(VCINTR::Align::getAlign(&OldInst)),
...
Instruction *SEVUtil::visitPHINode(PHINode &OldInst) {
  ...
  auto Phi =
      PHINode::Create(NewTy, OldInst.getNumIncomingValues(), "", (&OldInst)->getIterator());
...
Instruction *SEVUtil::visitAllocaInst(AllocaInst &OldInst) {
  ...
  auto *NewAlloca = new llvm::AllocaInst(NewTy, OldInst.getType()->getAddressSpace(),
...
Instruction *SEVUtil::visitCastInst(CastInst &OldInst) {
  ...
  auto NewVals = ValueCont{};
  std::tie(NewTy, NewVals) = getOperandsFreeFromSEV(OldInst);
  return CastInst::Create(OldInst.getOpcode(), NewVals[0], NewTy, "", (&OldInst)->getIterator());
}

Instruction *SEVUtil::visitLoadInst(LoadInst &OldInst) {
  Type *NewTy = nullptr;
  auto NewVals = ValueCont{};
  auto *NewLoad = new llvm::LoadInst(NewTy, NewVals[0], "", OldInst.isVolatile(),
    llvm::Align(VCINTR::Align::getAlign(&OldInst)),
    OldInst.getOrdering(), OldInst.getSyncScopeID(),
    (&OldInst)->getIterator());
  return NewLoad;
...
Instruction *SEVUtil::visitUnaryOperator(UnaryOperator &OldInst) {
  ...
  auto NewVals = ValueCont{};
  std::tie(NewTy, NewVals) = getOperandsFreeFromSEV(OldInst);
  return UnaryOperator::Create(OldInst.getOpcode(), NewVals[0], "", (&OldInst)->getIterator());
}
